...
object LambdaTermModel extends Properties("LambdaTermModel"):
  ...
  property("Abstraction should correctly store the bound variable and the body term.") = forAll: (v: Variable, t: Term) =>
    val abs = Abstraction(v, t)
    (abs.boundVariable == v) && (abs.body == t)

  property("Application should correctly link the left (function) and right (argument) terms.") = forAll: (l: Term, r: Term) =>
    val appl = Application(l, r)
    (appl.left == l) && (appl.right == r)

  property("Immutability") = forAll: (t: Term, v: Variable, replacement: Term) =>
...
object FreeVariables extends Properties("Free Variables"):
  ...
  property("FV of an abstraction with a different body: The set of free variables for λx. y should contain only y") = forAll: (t: Term, v: Variable) =>
    val termFV = Abstraction(v, t).freeVariables
    !termFV.contains(v) && termFV.subsetOf(t.freeVariables)

  property("FV of an application: The set of free variables for (x y) should be the union of free variables of x and y") = forAll: (x: Term, y:Term) =>
...
object Substitution extends Properties("Substitution"):
  ...
  property("Rule 1: Same variable substitution: [N/x]x should return N") = forAll: (x: Variable, n: Term) =>
    x.substitute(x, n) == n

  property("Rule 2: Different variable substitution: [N/x]a should return a when a != x") = forAll: (n: Term, x: Variable, a: Variable) =>
    (a != x) ==> (a.substitute(x, n) == a)

  property("Rule 3: Application substitution: [N/x](P Q) should return ([N/x]P [N/x]Q)") = forAll: (n: Term, x: Variable, appl: Application) =>
    appl.substitute(x, n) == Application(appl.left.substitute(x, n), appl.right.substitute(x, n))

  property("Rule 4: Shadowed abstraction substitution: [N/x](λx.P) should return λx.P (no changes)") = forAll: (n: Term, x: Variable, p: Term) =>
    Abstraction(x, p).substitute(x, n) == Abstraction(x, p)

  property("Rule 5: Constant abstraction substitution: [N/x](λy.P) should return λy.P if x is not free in P") = forAll: (n: Term, x: Variable, p: Term, y: Variable) =>
    (!p.freeVariables.contains(x)) ==> (Abstraction(y, p).substitute(x, n) == Abstraction(y, p))

  property("Rule 6: Capture-free abstraction substitution: [N/x](λy.P) should return λy.[N/x]P if y is not free in N") = forAll: (n: Term, x: Variable, p: Term, y: Variable) =>
